...
import { Trace } from './redis.js';

export interface ContextualDiagnosis {
  explanation: string;
  root_cause: string;
...
export async function analyzeTraceWithContextualAI(trace: Trace): Promise<ContextualDiagnosis> {
  ...
  try {
    ...
    console.warn('[ContextualAI] falling back to mock:', (err as Error).message);
    return buildMockDiagnosis(trace, start);
  }
}
...
function buildRootCause(trace: Trace): string {
...
function buildRecommendation(trace: Trace): string {
...
function buildMockDiagnosis(trace: Trace, start: number): ContextualDiagnosis {
