...
export interface FriendliResult {
  content: string;
  duration_ms: number;
  model: string;
  mock?: boolean;
...
export async function callFriendliAI(
  userPrompt: string,
  systemPrompt = 'You are a helpful assistant.',
  model = DEFAULT_MODEL
): Promise<FriendliResult> {
  ...
  try {
    const res = await axios.post(
      `${FRIENDLI_BASE}/chat/completions`,
      {
        model,
        messages: [
    ...
    console.warn('[FriendliAI] error:', (err as Error).message);
    return { content: mockResponse(userPrompt), duration_ms: Date.now() - start + 80, model, mock: true };
  }
}

export async function parseReplayChange(change: string): Promise<{
  step_to_fix: number;
  field: string;
  new_value: string;
  duration_ms: number;
}> {
  const { content, duration_ms } = await callFriendliAI(
    `Parse this trace replay instruction into a structured JSON edit. The trace has these step types: step 1=intent classification (FriendliAI), step 2=policy retrieval (ContextualAI), step 3=response generation (FriendliAI), step 4=session cache (Redis).

Instruction: "${change}"

Return only valid JSON with: step_to_fix (integer), field (string), new_value (string).
Example: {"step_to_fix": 2, "field": "document_version", "new_value": "2024"}`,
    'You are a JSON parser. Return only valid JSON, no explanation or markdown.'
  );
...
export function estimateCost(llmStepCount: number, totalSteps: number): string {
