...
export async function enrichFailureContext(
  ...
  try {
    ...
    if (!runId) return mockEnrichment(stepName, errorDescription, start);

    // Poll for results (with short timeout for demo)
    await new Promise((r) => setTimeout(r, 3000));

    const resultRes = await axios.get(
      `https://api.apify.com/v2/actor-runs/${runId}/dataset/items?token=${apiKey}&limit=5`,
      { timeout: 5000 }
    );

    const items = resultRes.data ?? [];
    const sources = items
      .flatMap((item: Record<string, unknown>) => (item.results as Array<{ title: string; url: string; snippet: string }>) ?? [])
      .slice(0, 4)
    ...
    console.warn('[Apify] Falling back to mock:', (err as Error).message);
    return mockEnrichment(stepName, errorDescription, start);
  }
}

function buildSearchQuery(stepName: string, error: string, query: string): string {
...
function buildSummary(stepName: string, sources: Array<{ title: string; snippet: string }>): string {
  if (sources.length === 0) return 'No external context found.';
...
function mockEnrichment(stepName: string, error: string, start: number): ApifyEnrichment {
