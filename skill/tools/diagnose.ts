...
function verifyCivic(): { ok: boolean; user_id: string; note: string } {
  const key = process.env.CIVIC_API_KEY;
  if (!key) return { ok: false, user_id: '', note: 'CIVIC_API_KEY not set — cannot authorize trace inspection' };
  return { ok: true, user_id: `civic_${key.slice(0, 8)}`, note: `Identity verified via Civic (${key.slice(0, 8)}…)` };
}

export async function diagnose(input: DiagnoseInput): Promise<string> {
  const { query } = input;
  if (!query?.trim()) return '❌ Describe the issue or provide a run_id.';

  const civic = verifyCivic();
  if (!civic.ok) return `🔐 Access denied: ${civic.note}`;
  ...
  const apify = await enrichFailureContext(
    ...
    'LLM answered using outdated information from its context',
    trace.user_query
  );
