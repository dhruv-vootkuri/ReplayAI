...
import { getTrace, saveTrace, resolveRunId, ReplayTrace } from '../utils/redis.js';
...
async function verifyCivic(): Promise<{ ok: boolean; user_id: string; note: string }> {
  const key = process.env.CIVIC_API_KEY;
  if (!key) return { ok: false, user_id: '', note: 'CIVIC_API_KEY not set. Replay requires Civic identity verification.' };
  try {
...
async function parseChange(change: string, context: string): Promise<{ find: string; replace: string }> {
  const result = await callFriendliAI(
    `Given this hotel knowledge base context and a requested change, return a JSON object with "find" (the exact text to replace) and "replace" (the new text).
...
Return ONLY valid JSON like: {"find": "exact text to find", "replace": "replacement text"}`,
    'You are a precise text editor. Return only valid JSON with find and replace fields. No explanation.'
  );

  try {
    const match = result.content.match(/\{[\s\S]*?\}/);
    return JSON.parse(match?.[0] ?? result.content) as { find: string; replace: string };
  } catch {
...
export async function replay(input: ReplayInput): Promise<string> {
  const { query, change } = input;
  if (!query?.trim()) return '❌ Describe which trace to replay or provide a run_id.';
  if (!change?.trim()) return '❌ Describe what you want to change. E.g. "what if the cancellation window was 24 hours instead of 48?"';

  // Civic: verify engineer identity before touching production traces
  const civic = await verifyCivic();
  if (!civic.ok) return `🔐 Replay blocked.\n\n${civic.note}`;
  ...
  const result = await callFriendliAI(
    trace.user_query,
    `You are the virtual concierge for The Grand Hotel. Answer the guest's question using only the information in the hotel reference guide below. Be warm and professional.\n\n${updatedContext}`
  );
  const replayDuration = Date.now() - replayStart + result.duration_ms;

  // Store the replay on the original trace in Redis
  const replayData: ReplayTrace = {
