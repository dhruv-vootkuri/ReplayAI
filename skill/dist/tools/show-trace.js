...
export async function showTrace(input) {
    const { query } = input;
    if (!query?.trim())
        return '❌ Describe what happened or provide a run_id.';
    ...
    const apify = await enrichFailureContext('LLM knowledge base', 'Agent responded using hotel knowledge base — checking for known issues with outdated information', trace.user_query);
